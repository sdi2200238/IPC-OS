...
1. **Μεταγλώττιση**: `make`
2. **Εκτέλεση**: `./ipc_os <command_file> <text_file> <max_children> <max_semaphores>`
...
```
Περιέχει:
- Ενα μύνημα, το οποίο χρησιμοποιούμε για να δώσουμε τις τυχαίες γραμμες κειμένου.
- Ενα `child_index`, που καθορίζει σε ποιό παιδί απευθύνεται το μύνημα
...
4. Σε κάθε κύκλο εκτέλεσης μεχρι να φτασει στην εντολή `EXIT`:
  ...
  - Επιλέγει κυκλικά ενα semaphore για το παιδί με το operation `semaphore = index % max_semaphores`, ωστε αν εχουμε λιγοτέρα semaphores απο παιδιά να μην υπαρχει προβλημα.
...
2. Ελέγχει ότι το μύνημα που έλαβε είναι για αυτό (αυτό γίνεται σε περίπτωση conflict που ο σημαφόρος είναι assigned σε πολλά παιδιά)
3. Διαβάζει το μήνυμα από την κοινή μνήμη και το επεξεργάζεται (π.χ., το εκτυπώνει στην κονσόλα).
4. Σηματοδοτεί τον γονέα όταν είναι έτοιμο για το επόμενο μήνυμα.
5. Τερματίζει ομαλά όταν λάβει σήμα τερματισμού (`kill`).
...
2. **Εκτέλεση**: Εκτελέστε το πρόγραμμα με τα απαραίτητα αρχεία εντολών και κειμένου, καθώς και τον μέγιστο αριθμό παιδιών.
  ```sh
  ./build/ipc_os <command_file> <text_file> <max_children> <max_semaphores>

  example:

  ./build/ipc_os ./files/config_10_10000.txt ./files/mobydick.txt 10 3
  ```
